...
class RobotEnv:
    ...
    def get_obs(self) -> Dict[str, Any]:
        ...
        # If no cameras are available, create dummy observations
        if not self._camera_dict:
            # Create dummy camera observations with zeros
            # Use standard names that agents expect
            observations["base_rgb"] = np.zeros((240, 320, 3), dtype=np.uint8)
            if self._save_depth:
                observations["base_depth"] = np.zeros((240, 320), dtype=np.uint16)
        else:
            # Read from actual cameras
            for name, camera in self._camera_dict.items():
                image, depth = camera.read()
                observations[f"{name}_rgb"] = image
                if self._save_depth:
