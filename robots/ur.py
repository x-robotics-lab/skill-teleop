...
class URRobot(Robot):
    ...
    def get_tcp_force(self) -> np.ndarray:
        """Get the current force/torque readings from the built-in wrist sensor.
        
        Returns:
            np.ndarray: 6D force/torque vector [Fx, Fy, Fz, Mx, My, Mz] in TCP frame
                       Forces in N, torques in Nm
        """
        return self.r_inter.getActualTCPForce()
